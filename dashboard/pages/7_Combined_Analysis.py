...
with col1:
    ...
    map_style = st.selectbox(
        "Map Quality",
        options=["Standard (Original Resolution)", "Smooth (Interpolated)"],
        index=0,
...
st.title("Identify, analyze, and predict algae bloom events in the Baltic Sea")
col1, col2 = st.columns(2)
...
tab1, tab2 = st.tabs(["📈 Bloom Timeline", "🌡️ Bloom-Favorable Environmental Conditions"])
...
with tab2:
    ...
    env_available = {
        ...
        'Rainfall': rain_data is not None
    }
    ...
    if rain_data is not None:
        rain_mean = rain_data.mean(dim=['latitude', 'longitude'], skipna=True)
